...
const int MIN_LINEAR_RPM	= 20;
const int MIN_ANGULAR_RPM	= 10;
...
const float MIN_LINEAR_SPEED = 20;
const float MIN_ANGULAR_SPEED = MAX_ANGULAR_SPEED / LEVELS;

const int LINEAR_RPM_VALUES[LEVELS]	= {20,40,60,80,100};
const int ANGULAR_RPM_VALUES[LEVELS]	= {10,25,40,55,70};

inline float to_meter_sec(int rpm)
{
    return (rpm * (2*M_PI)/60) * WHEEL_RADIUS;
}

inline float to_rad_sec(int rpm)
{
    return to_meter_sec(rpm) / (DIST_BETWEEN_WHEELS/2);
}

int main(int argc, char **argv)
{
    cv::Mat static_frame(height, width, CV_8UC3, cv::Scalar(153,76,0)); //BGR
    cv::Mat text_frame; 
	...
	{ 
	    case 'a':
		speed = to_rad_sec( ANGULAR_RPM_VALUES[speed_lvl - 1] );
		...
		break;
	    case 's':
		speed = to_meter_sec( -LINEAR_RPM_VALUES[speed_lvl - 1] );
		...
		//This value may change with versions
		//different than 4.x
		break;
	    case -23:
